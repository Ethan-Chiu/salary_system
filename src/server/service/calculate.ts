...
export class CalculateService {
    ...
    // 平/假日加班費
    async getOvertimeMoney(emp_no: string, period_id: number, overtime_type: "平日" | "假日") : Promise<number> {
        // get [Period] by id
        const period: Period = await (container.resolve(EHRService)).getPeriodById(period_id);
        ...
        // get overtime
        const overtime = (await (container.resolve(EHRService)).getOvertime(period_id)).find(
            (o: Overtime) => (o.emp_no === emp_no && o.type_name === overtime_type)
        );
        ...
        const salary_rate = (work_type === "外籍勞工") ? SALARY_RATE :
                            (work_status === "離職人員") ? 0 :
                            (work_status === "日薪") ? base_salary / 8 :
                            base_salary / 240;
    ...
    // 請假扣款
    async getLeaveDeduction(emp_no: string, period_id: number) : Promise<number> {
        // get [Period] by id
        const period: Period = await (container.resolve(EHRService)).getPeriodById(period_id);
        const period_start = period.start_date;
        const period_end = period.end_date ?? new Date();
        ...
        // get leave time
        const leavetime = (await (container.resolve(EHRService)).getHoliday(period_id)).find(
            (o: Overtime) => (o.emp_no === emp_no)
        );
        ...
        */
        ...
        if(isForeign()) {
            return Math.round(
                SALARY_RATE * t1 + SALARY_RATE/2 * t2
            );
        }
