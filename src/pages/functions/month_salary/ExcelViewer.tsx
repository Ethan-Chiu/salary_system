...
const handleExportExcel = async (datas: any, filename: string) => {
    ...
    if(datas) {
        datas.map((sheetdata: any, index: number) => {
            ...
            const worksheet = workbook.addWorksheet((name === "") ? "blank" : name);
            try {
...
const ExcelViewer: React.FC<ExcelViewerProps> = ({ sheets }) => {
    ...
    const findSheetIndexByName = (sheetName: string): number => {
        const index = sheets.findIndex((sheet) => sheet.sheetName === sheetName);
        return index;
    };
	...
	return (
        ...
        <div className="mb-4 grid grid-cols-4">
                    ...
                    <SelectContent>
                        <SelectGroup>
                            ...
                            {sheets.map((sheetdata: any) => {
                                return <SelectItem value={sheetdata.sheetName}>
                                    {sheetdata.sheetName}
                                </SelectItem>
                            })}
                        </SelectGroup>
            ...
            <div className="col-start-4 text-right">
                <Button variant={"outline"} onClick={() => {handleExportExcel(sheets, "test.xlsx");}}>Download</Button>
            </div>
        </div>
		<div className="excel-viewer-container p-4 w-full  bg-white border rounded shadow-md overflow-x-auto">
