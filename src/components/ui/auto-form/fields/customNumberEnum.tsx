...
export default function AutoFormNumberEnum({
	label,
	isRequired,
	field,
	fieldConfigItem,
	...
	// });
	const baseValues : Array<number> = (getBaseSchema(zodItem)._def as any).options.map((option: z.ZodLiteral<number>) => option._def.value);
	const options = baseValues.map((v: any) => {
		return { value: v, label: v };
	})
		...
		<FormItem>
			...
			<CustomSelect
				...
				onSelect={(selectedOption) => {
					...
					selectRef.current!.value = selectedOption ? selectedOption?.toString() : "Select an option";
...
interface CustomSelectProps {
	options: number[];
	onSelect: (selectedOption: number | null) => void; // Callback to handle the selected option
	defaultValue?: number;
}

function CustomSelect({ options, onSelect, defaultValue }: CustomSelectProps) {
	const [selectedOption, setSelectedOption] = useState<number | null>(
		defaultValue || null // "Select an option"
	);
